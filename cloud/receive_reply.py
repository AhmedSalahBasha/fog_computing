...
while keep_trying_to_connect:
    ...
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)        # Re-Create a socket object
    ...
    while True:
        try:
            ...
            if not has_handle(filename):
                ...
                return_msgs_list.append(return_msg)
                conn.send(return_msg.encode())
                print('==============================')
                print('[*] Number of Received Files From Edge:  ', len(received_data_list))
                print('[*] Number of Prediction Messages Sent To Edge:  ', len(return_msgs_list))
                #print("Prediction message has been sent sussessfully!")
        except KeyboardInterrupt:
            print('==============================')
            print('Connection Closed...')
            print('Cloud Machine Turned-Off')
            exit()
